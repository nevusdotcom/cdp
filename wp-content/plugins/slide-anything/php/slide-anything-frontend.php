		...
		if (($post_status != 'publish') || (count($metadata) == 0)) {
			...
			for ($i = 1; $i <= $slide_data['num_slides']; $i++) {
				...
				if ($valid_char_count == $actual_char_count) {
					...
					if (($slide_data['slide_min_height'] != '') && ($slide_data['slide_min_height'] != 0)) {
			...
			if (wp_script_is('jquery', 'done')) { // Only generate JQuery code if JQuery has been loaded
				...
				if (($slide_min_height != '') && ($slide_min_height != '0')) {
					...
					$output .= "				var slide_width = win_width / ".esc_attr($slide_data['items_width1']).";\n";
					$output .= "			} else if (win_width < 768) {\n";
					$output .= "				var slide_width = win_width / ".esc_attr($slide_data['items_width2']).";\n";
					$output .= "			} else if (win_width < 980) {\n";
					$output .= "				var slide_width = win_width / ".esc_attr($slide_data['items_width3']).";\n";
					$output .= "			} else if (win_width < 1200) {\n";
					$output .= "				var slide_width = win_width / ".esc_attr($slide_data['items_width4']).";\n";
					$output .= "			} else if (win_width < 1500) {\n";
					$output .= "				var slide_width = win_width / ".esc_attr($slide_data['items_width5']).";\n";
					$output .= "			} else {\n";
					$output .= "				var slide_width = win_width / ".esc_attr($slide_data['items_width6']).";\n";
					$output .= "			}\n";
